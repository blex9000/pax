...
fn run_print(parent: &gtk4::Window, markdown: &str, output_path: &Path) {
...

    {
        ...
        print_op.connect_draw_page(move |_, ctx, page_no| {
            ...
            for i in start..end {
                let block = &blocks[i];
                let h = block.measure(ctx, page_width);
                if y + h > usable_height {
                    // The block doesn't fit on the remaining page space —
                    // typically only happens for blocks taller than a
                    // whole page, which we've already let through. Draw
                    // anyway; the print clip will trim what overflows.
                }
...
fn draw_footer(
    ...
    let text = format!("{} / {}", page_no, total_pages);
    let desc = gtk4::pango::FontDescription::from_string("Sans 9");
    layout.set_font_description(Some(&desc));
    layout.set_text(&text);

    let (_w, h_pu) = layout.size();
    let h_pt = h_pu as f64 / gtk4::pango::SCALE as f64;
    cr.save().ok();
...
fn markdown_to_blocks(content: &str) -> Vec<Block> {
    ...
    for event in parser {
        match event {
            ...
            Event::Start(Tag::Strong) => append_inline(&mut b, "<b>"),
            Event::End(TagEnd::Strong) => append_inline(&mut b, "</b>"),
            Event::Start(Tag::Emphasis) => append_inline(&mut b, "<i>"),
            Event::End(TagEnd::Emphasis) => append_inline(&mut b, "</i>"),
            Event::Start(Tag::Strikethrough) => append_inline(&mut b, "<s>"),
            Event::End(TagEnd::Strikethrough) => append_inline(&mut b, "</s>"),
            Event::Code(s) => {
                append_inline(&mut b, "<tt>");
                append_inline(&mut b, &escape(&s));
                append_inline(&mut b, "</tt>");
            }
            ...
            Event::SoftBreak => append_inline(&mut b, " "),
            Event::HardBreak => append_inline(&mut b, "\n"),
            Event::Rule => b.blocks.push(Block::Rule),
            Event::Start(Tag::Link { .. }) => {}
            Event::End(TagEnd::Link) => {}
            Event::Start(Tag::Image { .. }) => append_inline(&mut b, "[image]"),
...
fn append_inline(b: &mut BlockBuilder, s: &str) {
    if matches!(b.inline_target, InlineTarget::None | InlineTarget::CodeBlock) {
        return;
    }
