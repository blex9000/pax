...
pub fn run_app(workspace: Option<Workspace>, config_path: Option<&Path>) -> Result<()> {
    // Force GTK built-in dialogs instead of xdg-desktop-portal ones.
    // Portal color pickers (xapp, gtk, gnome) often ignore the initial
    // color parameter passed via choose_rgba.
