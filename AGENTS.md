...
Pax is a Rust workspace rooted at `Cargo.toml`. The main code lives under `crates/`: `tp-core/` for shared models and config loading, `tp-db/` for SQLite persistence and migrations, `tp-gui/` for the GTK4/libadwaita app, and `tp-cli/` for the `pax` binary entry point. Use Cargo package names (`pax-core`, `pax-gui`, `pax-db`, `pax`) in commands, even though the on-disk directories are prefixed `tp-`. Supporting files live in `resources/` (CSS, fonts, icons), `migrations/` (SQL schema), `docs/`, and `scripts/`.

## Build, Test, and Development Commands
Use `cargo build` for the default Linux debug build and `cargo build --release` for optimized binaries. Run the app locally with `cargo run -- new "scratch"` or launch an existing workspace with `cargo run -- launch /path/to/workspace.json`. Execute the full test suite with `cargo test`; scope to a crate or feature area when iterating, for example `cargo test --package pax-gui file_watcher`. Linux packaging uses `./scripts/build-appimage.sh`; macOS bundle builds use `./scripts/build-macos.sh`.
